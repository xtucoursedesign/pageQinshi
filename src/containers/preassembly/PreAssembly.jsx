...
import {reqMain, reqProject, reqAssembly, reqPreAssembly} from '../../api';
...
export default class Layoff extends Component {
    ...
    getAllAssemblyByProId = async () => {
        let result = await reqPreAssembly({method: 'getAllPreassemblyByProId', proid: this.props.match.params.pid});
        // console.log(result);
        if(result.status === 2){
            message.warning(result.msg, 1);
        }else{
            ...
            // console.log(main)
            let preassemblies = result.data.preassemblies;
            // console.log(preassemblies)
            preassemblies.map((item) => {
    ...
    // 切换状态
    updatePreAssemblyState = async ({paid, complete}) => {
        // console.log(paid, complete);
        // return;
        let preassemblies = [...this.state.preassemblies];
        let con = complete === 1 ? 0 : 1;
        let result = await reqPreAssembly({method: 'updatePreassemblyState', complete: con, paid});
        message.success(result.msg, 1);
        let index = preassemblies.findIndex((item) => item.paid === paid);
        preassemblies[index].complete = con;
        this.setState({preassemblies});
    }

    removeAssembly = async (paid) => {
        // console.log(paid);
        let result = await reqPreAssembly({method: 'removePreassembly', paid});
        message.success(result.msg, 1);
        let preassemblies = [...this.state.preassemblies];
        preassemblies = preassemblies.filter(item => item.paid !== paid);
        this.setState({preassemblies});
    }
    ...
    render() {
        const {preassemblies, isLoading, proname, visible, require} = this.state;
        const columns = [
            {
              title: '编号',
              dataIndex: 'paid',
            ...
            },
            {
                ...
                render: item => {
                    if(item.complete === 1){
                        return (
                            <div>
                                <Button type="primary" danger onClick={() => {this.updatePreAssemblyState(item)}}>切换为未完成</Button><br />
                                <span>已完成</span>
                            </div>
                        )
                    }else{
                        return (
                            <div>
                                <Button type="primary" onClick={() => {this.updatePreAssemblyState(item)}}>切换为已完成</Button><br />
                                <span>未完成</span>
                            </div>
                        )
                    }
            ...
            },
            {
                ...
                render: item => (
                    <div>
                        ...
                        <Button style={{marginRight: '10px'}} type="primary" onClick={() => this.props.history.push(this.props.history.location.pathname + "/" + item.paid)}>修改</Button>
                        <Popconfirm
                          title={`确认删除?`}
                          onConfirm={() => this.removeAssembly(item.paid)}
                          okType="danger"
                          okText="确认"
                          cancelText="取消"
                        >
        ...
        return (
            ...
            >
                <div style={{height: "100%"}} key="projectkey">
                    <Card 
                        title={<Button type="link" size="large" icon={<LeftOutlined />} onClick={() => this.props.history.goBack()}>组装管理</Button>}
                        extra={<Button type="primary" icon={<PlusOutlined />} onClick={() => this.props.history.push(this.props.history.location.pathname + "/addpreassembly")}>添加组装</Button>}
                        style={{height: "100%"}}
                    >
